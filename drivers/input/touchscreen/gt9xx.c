...
static struct goodix_ts_data *gtp_ts;
...
#endif
static int goodix_ts_early_suspend_pm (void);
static int goodix_ts_late_resume_pm(void);
...
static  void get_gtp_mounle_fw_path(void)
{
	...
	strcat(gtp_sdcard_path_temp,gtp_file_name);
	strcat(gtp_etc_path_temp,gtp_file_name);
	strcpy(gtp_sdcard_path,gtp_sdcard_path_temp);
	strcpy(gtp_etc_path,gtp_etc_path_temp);
...
len += sprintf(page + len, "chip type : %s\n", "GOODIX_GT9");	
...
#endif
	len += sprintf(page + len, "sensor id : %d\n", gtp_ts->sensor_id);
	len += sprintf(page + len, "fw version : %04x\n", gtp_ts->version);
	...
	printk("etc path:%s\n",gtp_etc_path);
		if(get_gtp_mounle_fw_info(&fw_head)>=0)	{
			...
			if((((fw_head.vid&0xff)<<8)|((fw_head.vid&0xff00)>>8)) > gtp_ts->version)
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	touchscreen_suspend_pm=goodix_ts_early_suspend_pm;
	touchscreen_resume_pm=goodix_ts_late_resume_pm;
...
static int goodix_ts_late_resume_pm(void)
{
	...
	{
		GTP_INFO("IC is not sleep no system resume.");
		return 0;
	}
    ret = gtp_wakeup_sleep(ts);
...
static int __devinit goodix_ts_init(void)
{
    ...
    ret = i2c_add_driver_async(&goodix_ts_driver);
    return ret; 
}
