...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	dev_info(phy->dev, "USB in low power mode, lpm_flags:%lx\n", motg->lpm_flags);
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	else if (motg->chg_type == USB_WIRELESS_CHARGER)//zte
		charger_type = POWER_SUPPLY_TYPE_WIRELESS;//zte
	else
...
static int msm_otg_set_host(struct usb_otg *otg, struct usb_bus *host)
{
	...
	if (motg->pdata->mode == USB_HOST || otg->gadget) {
		pr_info("msm_otg: runtime_get_sync %s: %d\n", __func__, __LINE__);
		pm_runtime_get_sync(otg->phy->dev);
		queue_work(system_nrt_wq, &motg->sm_work);
	}
...
static const char *chg_to_string(enum usb_chg_type chg_type)
{
	...
	case USB_WIRELESS_CHARGER:	return "USB_WIRELESS_CHARGER";//zte
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_WAIT_FOR_DCD:
		...
		/*zte:detect wireless charge*/
		{
			extern bool is_only_wireless_charge(void);
			if (is_only_wireless_charge()) {
				pr_info("found wireless charge only\n");
				msm_chg_block_off(motg);
				motg->chg_state = USB_CHG_STATE_DETECTED;
				motg->chg_type = USB_WIRELESS_CHARGER;
				queue_work(system_nrt_wq, &motg->sm_work);
				return;
			}
		}
...
static void msm_invalid_chg_work(struct work_struct *w)
{
	...
	printk(KERN_ERR "usb msm_otg schedule %s\n",__func__);
	msm_otg_notify_charger(motg, IDEV_CHG_MIN); //500mA
}
/*end*/
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		if (!test_bit(B_SESS_VLD, &motg->inputs) &&
				test_bit(ID, &motg->inputs)) {
			pr_info("msm_otg: runtime_put_noidle %s: %d\n", __func__, __LINE__);
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_PROPRIETARY_CHARGER:
					if (motg->chg_type == USB_WIRELESS_CHARGER)//zte
